...
class DWTFunction_2D(Function):
    ...
    def backward(ctx, grad_LL, grad_LH, grad_HL, grad_HH):
        ...
        return grad_input, None, None, None, None
#改成提出LH HL HH的高频部分
class IDWT_2D_High(Module):
    def __init__(self, wavename):
        super(IDWT_2D_High, self).__init__()
        wavelet = pywt.Wavelet(wavename)
        self.band_low = wavelet.dec_lo
        self.band_low.reverse()
        self.band_high = wavelet.dec_hi
    ...
    def forward(self, LH, HL, HH):
        assert len(LH.size()) == len(HL.size()) == len(HH.size()) == 4
        self.input_height = LH.size()[-2] + HH.size()[-2]
        self.input_width = LH.size()[-1] + HH.size()[-1]
        self.get_matrix()
        return IDWTFunction_2D_High.apply(LH, HL, HH, self.matrix_low_0, self.matrix_low_1, self.matrix_high_0,
                                          self.matrix_high_1)


class IDWTFunction_2D_High(Function):
    @staticmethod
    def forward(ctx, input_LH, input_HL, input_HH,
                matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1):
        ctx.save_for_backward(matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1)
        L = torch.matmul(input_LH, matrix_High_1.t())
        H = torch.add(torch.matmul(input_HL, matrix_Low_1.t()), torch.matmul(input_HH, matrix_High_1.t()))
    ...
    def backward(ctx, grad_output):
        ...
        grad_HL = torch.matmul(grad_H, matrix_Low_1)
        grad_HH = torch.matmul(grad_H, matrix_High_1)
        return grad_LH, grad_HL, grad_HH, None, None, None, None